...
def build_cnn(input_var=None, W_init=None, n_layers=(4, 2, 1), n_filters_first=32, imSize=32, n_colors=3):
    ...
    # Input layer
    network = tf.placeholer(tf.float32,shape=(None, imSize, imSize, n_colors,),name='Input')
    if W_init is None:
        for i, s in enumerate(n_layers):
            ...
            for l in range(s):
                with tf.variable_scope("conv"+str(i)+"_"+str(l)):
...
def build_convpool_mix(input_vars, nb_classes, GRAD_CLIP=100, imSize=32, n_colors=3, n_timewin=3,train=False):
    """
    ...
    with tf.variable_scope("CONVFINAL"):
        weights = tf.get_variable("weights", [3,1,tf.shape(convnets[0])[1],64],
                                  initializer=tf.contrib.layers.xavier_initializer_conv2d())
    ...
    cell = tf.nn.rnn_cell.LSTMCell(num_hidden, use_peepholes=True, cell_clip=GRAD_CLIP,initializer=tf.constant_initializer(0.0),
                state_is_tuple=True)
    lstm = tf.nn.dynamic_rnn(cell,convpool,scope="LSTM")
    ...
    lstm_out = tf.slice(lstm,[0,n_timewin-1,0],[-1,1,-1])
    ...
    with tf.variable_scope("FC1"):
        weights = tf.get_variable("weights",[dim,512],tf.contrib.layers.xavier_initializer())
    ...
    with tf.variable_scope("FC2"):
        weights = tf.get_variable("weights",[512,nb_classes],tf.contrib.layers.xavier_initializer())
...
if __name__ == '__main__':
    ...
    network = build_convpool_mix(X, 3, 90, train)
